...
const envCdShim = "__KWT_CD_SHIM"

// isCdShimActive reports whether the current process is running under the
// shell integration shim for cd/add. The wrapper sets __KWT_CD_SHIM=1 before
...
func runCd(cmd *cobra.Command, args []string) error {
	...
	if !cfg.Cd.LaunchShell && !isCdShimActive() {
		return fmt.Errorf(`'kwt cd' requires shell integration when cd.launch_shell is false.
...
Or, to use the old behavior (launching a new shell), run:
  kwt config set cd.launch_shell true`)
	}
