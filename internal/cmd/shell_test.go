...
func TestUpdateEnvVar(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			key:      "KWT_SHELL_DEPTH",
			value:    "1",
			expected: []string{"PATH=/usr/bin", "KWT_SHELL_DEPTH=1"},
		},
		{
			name:     "update existing variable",
			env:      []string{"PATH=/usr/bin", "KWT_SHELL_DEPTH=1"},
			key:      "KWT_SHELL_DEPTH",
			value:    "2",
			expected: []string{"PATH=/usr/bin", "KWT_SHELL_DEPTH=2"},
		},
		{
			name:     "empty env",
			env:      []string{},
			key:      "KWT_SHELL_DEPTH",
			value:    "1",
			expected: []string{"KWT_SHELL_DEPTH=1"},
		},
	}
