...
func TestRenderCommands(t *testing.T) {
	data := &TemplateData{
		...
		Hash:       "a1b2c3d4",
		Path:       "/tmp/worktrees/kwt/feature-new-ui",
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			wantRendered: []string{
				"echo feature/new-ui /tmp/worktrees/kwt/feature-new-ui",
				"zellij action new-tab --name feature/new-ui -- nvim .",
				"mkdir -p /tmp/worktrees/kwt/feature-new-ui/.local",
			},
		...
		},
		{
			...
			wantRendered:  []string{"", "echo feature/new-ui", "echo /tmp/worktrees/kwt/feature-new-ui"},
			wantErrSubstr: []string{"execute template", "", ""},
		},
	}
