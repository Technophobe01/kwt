...
func TestRunPostWorktreeSetup_RendersTemplateVariables(t *testing.T) {
	...
	setting := models.RepositorySetting{
		...
		SetupCommands: []string{
			...
			"echo host={{.Host}} owner={{.Owner}} repo={{.Repository}} full={{.FullPath}}",
		},
	}
	m := buildManagerWithRepoSetting(git, setting)

	exec := newRecordingExecutor()
	results := m.runPostWorktreeSetupWithExecutor(context.Background(), exec, "feature/new-ui", "/tmp/worktrees/kwt/feature-new-ui")
	...
	got := exec.rendered()
	wantContains := []string{
		"branch=feature/new-ui path=/tmp/worktrees/kwt/feature-new-ui",
		"host=github.com owner=test-user repo=test-repo full=github.com/test-user/test-repo",
	}
