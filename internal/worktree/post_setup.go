...
// When the repository has no resolvable origin URL, Host/Owner/Repository/FullPath/Hash
...
func buildSetupTemplateData(git GitInterface, branch, worktreePath string) *template.TemplateData {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[kwt] warning: origin URL unavailable, Host/Owner/Repository/FullPath/Hash will be empty: %v\n", err)
		return data
	}

	repoInfo, err := url.ParseRepositoryURL(repoURL)
	if err != nil {
		fmt.Fprintf(os.Stderr, "[kwt] warning: failed to parse repository URL %q: %v\n", repoURL, err)
		return data
	}
	...
	data.FullPath = repoInfo.FullPath
